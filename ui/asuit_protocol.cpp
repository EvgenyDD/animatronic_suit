...
const unsigned long sw_crc32_table[256] =
{
...
uint32_t stm32_sw_crc32_by_byte(uint8_t *pBuffer, uint32_t NumOfByte)
{
...
int Flasher::flash_process()
{
    ...
    AnyContainer data;
    for(uint32_t k = 0; k < 4; k++) data.append<uint8_t>(fw[k]);
#if 1
    data.append<uint32_t>(fw.size());
    data.append<uint32_t>(stm32_sw_crc32_by_byte(&fw[0], fw.size()));
